...
class NotionClient:
    ...
    def query_by_regcode(self, regcode: str):
        ...
        payload = {
            "filter": {"property": "Registrikood", "number": {"equals": regcode_num}}
        }
...
def _prepare_notion_properties(
    ...
    if not email_val:
        empty_fields.append("E-post")
    if not tel_val:
        empty_fields.append("Tel. nr")
    if not veeb_val:
        empty_fields.append("Veebileht")
    if not linkedin_val:
        empty_fields.append("LinkedIn")
    if not clean_value(company.get("asukoht_ettevotja_aadressis")):
        empty_fields.append("Aadress")
    if not clean_value(company.get("tegevusvaldkond")):
        empty_fields.append("Tegevusvaldkond")
    if not clean_value(company.get("pohitegevus")):
        empty_fields.append("Põhitegevus")
    ...
    properties = {
            ...
            "rich_text": [
                {
                    "text": {
                        "content": clean_value(
                            company.get("asukoht_ettevotja_aadressis")
                        )
                        or ""
                    }
                }
        ...
        "Tegevusvaldkond": {
            "rich_text": [
                {"text": {"content": clean_value(company.get("tegevusvaldkond")) or ""}}
            ]
        },
        "Põhitegevus": {
            "rich_text": [
                {"text": {"content": clean_value(company.get("pohitegevus")) or ""}}
            ]
        },
    }
...
def load_company_data(regcode: str, config: dict) -> dict:
    ...
    properties, empty_fields, company_name = _prepare_notion_properties(
        company, regcode
    )
    ...
    flat_data = {
        ...
        "Maakond": (
            properties["Maakond"]["multi_select"][0]["name"]
            if properties["Maakond"]["multi_select"]
            else ""
        ),
...
def _reconstruct_notion_properties(flat_data: dict) -> Tuple[Dict[str, Any], List[str]]:
    ...
    properties["Tegevusvaldkond"] = {
        "rich_text": [{"text": {"content": tegevusvaldkond or ""}}]
    }
    properties["Põhitegevus"] = {
        "rich_text": [{"text": {"content": pohitegevus or ""}}]
    }

    # 4. Multi-Select (Maakond)
    maakond = flat_data.get("Maakond")
    properties["Maakond"] = (
        {"multi_select": [{"name": maakond}]} if maakond else {"multi_select": []}
    )
...
def autofill_page_by_page_id(page_id: str, config: dict):
    ...
    try:
        ...
        if (
            reg_prop
            and reg_prop.get("type") == "number"
            and reg_prop.get("number") is not None
        ):
        ...
        if not company:
            st.warning(
                f"⚠️ Company with registration code {regcode} not found in CSV data. Cannot autofill."
            )
            return
...
def sync_form_to_notion(data_to_sync):
    ...
    updated_notion_properties, empty_fields = _reconstruct_notion_properties(
        data_to_sync["flat_data"]
    )
    ...
    with st.form(key="sync_data_form"):
        ...
        for key, value, is_mandatory in fields_to_display:
            ...
            if key == "Maakond":
                ...
                default_index = (
                    available_counties.index(value)
                    if value in available_counties
                    else (
                ...
                updated_data[key] = st.text_input(
                    label, value=value if value is not None else "", key=f"form_{key}"
                )
        ...
        sync_submitted = col_sync.form_submit_button(
            label="Fill and Sync (Täida ja Sünkrooni)", type="primary"
        )
        cancel_clicked = col_cancel.form_submit_button(label="Cancel (Katkesta)")

        if sync_submitted:
            if not updated_data.get("Registrikood") or not updated_data.get("Nimi"):
                st.error(
                    "ERROR: Registration Code and Company Name are mandatory fields."
                )
            else:
...
def main():
    ...
    st.set_page_config(
        page_title="Business Register Synchronizer to Notion", layout="centered"
    )
    st.title("🇪🇪 Business Register Data Synchronization to Notion")
    st.caption(
        "Automated tool for fetching company data and loading it into a Notion database."
    )
    ...
    tab1, tab2 = st.tabs(
        ["New Sync (Registration Code)", "Autofill Existing (Page ID)"]
    )
    ...
    with tab1:
        ...
        if (
            "data_to_sync" not in st.session_state
            or st.session_state.data_to_sync is None
        ):

            regcode = st.text_input(
                "Enter Company Registration Code (e.g., 10000000):", key="regcode_input"
            )
        ...
        if (
            "data_to_sync" in st.session_state
            and st.session_state.sync_mode == "new"
            and st.session_state.data_to_sync is not None
        ):
    ...
    with tab2:
        ...
        st.markdown(
            "This mode supplements an existing Notion page that already has the **Registrikood** field."
        )
        st.info(
        ...
        page_id = st.text_input(
            "Enter Notion Page ID (32-character string):", key="page_id_input"
        )
